...
function search(ele) {
  ...
  if(event.key === "Enter") {
    var getNation = ele.value.trim();
    var realCountry = newCountries.indexOf(getNation.toLowerCase());
    //The array will store and check if the country has already been guessed
    if(existingArr.includes(countries[realCountry])){
      ...
      if (newCountries.includes(getNation.toLowerCase())) {
        existingArr.push(countries[realCountry]);
...
function colorChange(){
  var natns = existingArr[existingArr.length-1];//last index of the array which is the latest country user guessed
